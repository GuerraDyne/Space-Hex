...
async function testConcurrentGames() {
  ...
  try {
    ...
    // START THE FIRST GAME to lock it
    console.log("\nStarting first game...");
    room1.send("ready", {});
    room2.send("ready", {});
    ...
    await new Promise(resolve => {
      room1.onMessage("gameStarted", () => {
        console.log("Game 1 started!");
        resolve();
      });
    });
    
    console.log("\nCreating second game (first game is now locked)...");
    ...
    if (room1.roomId !== room3.roomId) {
      console.log("\n🎉 SUCCESS! Two separate games are running concurrently!");
      console.log("  Game 1 ID:", room1.roomId);
      console.log("  Game 2 ID:", room3.roomId);
    } else {
      console.log("\n❌ FAILED: All players joined the same room");
